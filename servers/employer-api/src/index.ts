...
const getFilePath = (filename: keyof typeof FILES) => path.join(TEMP_DIR, FILES[filename]);

// Helper function to execute heimdalljs commands
async function executeHeimdallCommand(command: string, args: string[]): Promise<string> {
...
app.post('/verify-income', async (req: Request, res: Response) => {
  try {
...
try {
  ...
  process.exit(1);
      ...
      if (execError.stdout) console.error('Command output:', execError.stdout);
      if (execError.stderr) console.error('Command error:', execError.stderr);
      throw execError;
    }
  } catch (error: any) {
